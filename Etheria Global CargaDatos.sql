/*nota, los scripts actualmente no son idenpotentes, eso quiere decir que si se ejecutan mas de una vez da error*/

/* ============================================================
   SOPORTE: acciones + tablas del sistema
   ============================================================ */
...
/* ============================================================
   CATÁLOGOS BASE: usuarios, roles, permisos, niveles, tipos
   ============================================================ */
...
/* ============================================================
   GEOGRAFIA: 6 países, divisiones y dirección HUB
   ============================================================ */
...
/* ============================================================
   PROVEEDORES, CONTACTOS Y CENTRO LOGISTICO
   ============================================================ */
...
/* ============================================================
   FINANZAS: monedas, tipos de cambio, permisos e impuestos
   ============================================================ */
...
begin
    begin
        ...
        if v_m_usd is null then
            select monedaid into v_m_usd from monedas where nombremoneda = 'USD' limit 1;
        end if;
        ...
        on conflict do nothing
        returning monedaid into v_m_nio;
        ...
        on conflict do nothing
        returning monedaid into v_m_col;
        ...
        on conflict do nothing
        returning monedaid into v_m_per;
        ...
        on conflict do nothing
        returning monedaid into v_m_mex;
        ...
        loop
            select tipopermisoid
            into v_tipo_permiso
            from tipospemisos
            where nombretipopermiso = r.nombre
            limit 1;
        ...
        /* Permisos de importacion: 2 por país */
        for r in
            select * from (values
                ('Colombia', v_m_col, v_tc_usd_col, 1),
                ('Colombia', v_m_col, v_tc_usd_col, 2),
                ('Peru',     v_m_per, v_tc_usd_per, 1),
                ('Peru',     v_m_per, v_tc_usd_per, 2),
                ('Mexico',   v_m_mex, v_tc_usd_mex, 1),
                ('Mexico',   v_m_mex, v_tc_usd_mex, 2),
                ('Chile',    v_m_chl, v_tc_usd_chl, 1),
                ('Chile',    v_m_chl, v_tc_usd_chl, 2),
                ('Nicaragua',v_m_nio, v_tc_usd_nio, 1),
                ('Nicaragua',v_m_nio, v_tc_usd_nio, 2)
            ) as x(pais, monedaid, tipocambioid, idx)
        loop
            select paisid into v_pais
            from paises
            where nombrepais = r.pais
            limit 1;

            select tipopermisoid into v_tipo_permiso
            from tipospemisos
            order by tipopermisoid
            offset case when r.idx = 1 then 0 else 1 end
            limit 1;

            if not exists (
                select 1
                from permisosimportacion
                where paisid = v_pais
                  and tipoPermisoID = v_tipo_permiso
            ) then
                ...
                values (
                    ...
                    r.tipocambioid,
                    case
                        when r.pais = 'Colombia' then 3950.000000
                        when r.pais = 'Peru' then 3.750000
                        when r.pais = 'Mexico' then 17.200000
                        when r.pais = 'Chile' then 930.000000
                        else 36.500000
                    end,
                    case when r.idx = 1 then 'Permiso sanitario' else 'Permiso etiquetado' end,
                    'Permiso de importacion para Etheria Global',
                    'https://docs.etheria.global/permisos',
                    case
                        when r.pais = 'Colombia' then 250000.00
                        when r.pais = 'Peru' then 180.00
                        when r.pais = 'Mexico' then 900.00
                        when r.pais = 'Chile' then 120000.00
                        else 2000.00
                    end,
                    true
                );
        ...
        /* Impuestos por país: 1 porcentual + 1 fijo */
        for r in
            select * from (values
                ('Colombia', v_m_col, v_tc_usd_col),
                ('Peru',     v_m_per, v_tc_usd_per),
                ('Mexico',   v_m_mex, v_tc_usd_mex),
                ('Chile',    v_m_chl, v_tc_usd_chl),
        ...
        loop
            ...
            if not exists (
                select 1 from impuestospais
                where paisid = v_pais and nombre = 'IVA'
            ) then
                insert into impuestospais(
                    paisid, usuariomodificacion, monedaid, tipocambioid, tipocambio,
                    nombre, valor, tipo, fechainicio, fechafin, activo
                )
                values (
                    v_pais, v_admin, r.monedaid, r.tipocambioid, 1.000000,
                    'IVA', 13.00, 'porcentaje', current_timestamp, null, true
                );
            end if;

            if not exists (
                select 1 from impuestospais
                where paisid = v_pais and nombre = 'Arancel'
            ) then
                insert into impuestospais(
                    paisid, usuariomodificacion, monedaid, tipocambioid, tipocambio,
                    nombre, valor, tipo, fechainicio, fechafin, activo
                ...
                values (
                    v_pais, v_admin, r.monedaid, r.tipocambioid, 1.000000,
                    'Arancel', 25.00, 'monto_fijo', current_timestamp, null, true
                );
            end if;
        end loop;
        ...
        if not exists (select 1 from balanceneto) then
            ...
            values (0.000000, current_timestamp);
        end if;

        call sp_log_proceso(
            'sp_cargar_finanzas_etheria',
            'finanzas',
            'OK',
            'Monedas, tipos de cambio, permisos e impuestos cargados',
            0
        );
    exception
        when others then
...
/* ============================================================
   PRODUCTOS, LOTES, INVENTARIO, HISTORIAL DE PRECIOS
   ============================================================ */
...
begin
    begin
        ...
        for i in 1..100 loop
            ...
            select nombrecategoriap into v_cat_name
            from categorias
            where categoriaid = v_category_id
            limit 1;
            ...
            values (
                ...
                v_admin,
                v_provider_id,
                'Producto ' || lpad(i::text, 3, '0') || ' - ' || v_cat_name,
                'Producto exótico de alta gama para el catalogo Etheria Global',
            ...
            /* Características */
            v_char1 := v_char_ids[1];
            v_char2 := v_char_ids[2];
            v_char3 := v_char_ids[3];

            insert into valorcaracteristicas(productoid, caracteristicaid, valor, activo)
            values
            (
                v_product_id, v_char1,
                case (i % 5)
                    when 0 then 'Vegetal'
                    when 1 then 'Semilla'
                    when 2 then 'Flor'
            ...
            (
                v_product_id, v_char2,
                case v_category_id % 7
                    when 0 then 'Hidratante'
                    when 1 then 'Digestivo'
                    when 2 then 'Relajante'
                    when 3 then 'Purificante'
                    when 4 then 'Nutritivo'
                    when 5 then 'Equilibrante'
                    else 'Aromatico'
                end,
                true
            ),
            (
                v_product_id, v_char3,
                case (i % 3)
                    when 0 then '250 ml'
                    when 1 then '500 ml'
                    else '100 g'
            ...
            insert into historialpreciosproducto(
                productoid, precio, monedaid, fechainicio, fechafin, activo
            )
            values (
                v_product_id, v_price, v_usd, v_today, null, true
            );

            v_qty := 40 + (i % 25);

            insert into lotes(
                productoid, cantidadproductoloteinicial, cantidadproductolotedisponible,
                fechafabricacion, fechavencimiento
            )
            values (
                v_product_id, v_qty, v_qty,
                v_today - ((i % 30) || ' days')::interval,
                v_today + interval '365 days'
            )
            returning loteid into v_lote_id;

            insert into inventarios(
                loteid, usuariomodificacion, cantidaddisponible, ultimaactualizacion
            )
            values (
                v_lote_id, v_admin, v_qty, v_today
            );

            insert into movimientosinventario(
                loteid, usuariomodificacion, tipomovimientoinvetarioid, cantidad, fecha
            )
            values (
                v_lote_id, v_admin, v_tipo_entrada, v_qty, v_today
            );
        end loop;

        call sp_log_proceso(
            'sp_cargar_productos_etheria',
            'productos',
            'OK',
            '100 productos, lotes e inventario inicial cargados',
            0
        );
    exception
        when others then
...
/* ============================================================
   ORDENES, DETALLES, IMPUESTOS, PERMISOS, DESCUENTOS, CUENTA
   ============================================================ */
...
begin
    begin
        ...
        select estadoid into v_estado_pendiente
        from estadosordenes where nombreestadoorden = 'Pendiente' limit 1;

        select estadoid into v_estado_completada
        from estadosordenes where nombreestadoorden = 'Completada' limit 1;

        select tipoordenid into v_tipo_importacion
        from tiposorden where nombre = 'Importacion' limit 1;

        select estadotransaccionid into v_estado_tx_pendiente
        from estadotransacciones where nombreestadotransac = 'Pendiente' limit 1;

        select estadotransaccionid into v_estado_tx_aprobada
        from estadotransacciones where nombreestadotransac = 'Aprobada' limit 1;

        select tipoid into v_tipo_tx_importacion
        from tipotransacciones where nombretipotransac = 'Compra importacion' limit 1;
        ...
        select centrologisticoid into v_hub_centro
        from centroslogisticos
        where nombrecentrologistico = 'HUB Caribe Nicaragua'
        limit 1;
        ...
        select array_agg(tipocambioid order by tipocambioid)
        into v_rate_ids
        from tiposcambio
        where moneda1id = v_usd;

        select array_agg(tipocambio order by tipocambioid)
        into v_rate_values
        from tiposcambio
        where moneda1id = v_usd;

        /* 5 órdenes, una por país proveedor */
        for i in 1..5 loop
            v_total_order := 0;
            ...
            values (
                ...
                (select d.direccionid
                 ...
                 where pr.proveedorid = v_provider_ids[i]
                 limit 1),
                v_hub_direccion,
                v_usd,
                v_rate_id,
            ...
            for j in 1..2 loop
                ...
                select precio
                into v_price
                from historialpreciosproducto
                where productoid = v_product_id
                order by fechainicio desc
                limit 1;

                select i2.impuestoid, i2.valor
                into v_impuesto_id, v_tax_local
                ...
                where p.productoid = v_product_id
                  and i2.paisid = pa.paisid
                order by i2.impuestoid
                limit 1;

                select pi.permisoid, pi.costo
                into v_permiso_id, v_permiso_local
                ...
                join paises pa on pa.paisid = dg.paisid
                where p.productoid = v_product_id
                  and pi.paisid = pa.paisid
                order by pi.permisoid
                limit 1;

                v_qty := case when j = 1 then 2 else 3 end;
                v_base := round(v_price * v_qty, 6);

                if v_tax_local is null then
                    v_tax_usd := 0;
                else
                    v_tax_usd := round((v_base * v_tax_local / 100.0), 6);
                end if;

                if v_permiso_local is null then
                    v_permiso_usd := 0;
                else
                    v_permiso_usd := round(v_permiso_local / v_rate_value, 6);
                end if;
                ...
                if v_discount > 0 then
                    ...
                    values (
                        v_detail_id, v_usd, v_rate_id, v_rate_value,
                        'Descuento promocional Etheria',
                        v_discount
                    );
                end if;
            end loop;
            ...
            update ordenes
            set preciofinal = v_total_order
            where ordenid = v_order_id;

            insert into trazabilidadorden(
                ordenid, centrologisticoid, direccionid, usuariomodificacion, estadoid, fecha
            )
            values (
                v_order_id,
                v_hub_centro,
            ...
            insert into estadoscuenta(
                ordenid, usuariomodificacion, tipomovimiento, estado,
                monedaid, tipocambioid, tipocambio, monto, fecharegistro, checksum
            )
            values (
                v_order_id,
                v_admin,
            ...
            insert into transacciones(
                monedaid, usuariomodificacion, tipoid, estadotransaccionid,
                ordenid, tipocambioid, tipocambio, monto, descripcion, fecha, checksum
            )
            values (
                v_usd,
                v_admin,
                v_tipo_tx_importacion,
                case when i % 2 = 0 then v_estado_tx_aprobada else v_estado_tx_pendiente end,
                v_order_id,
                v_rate_id,
                v_rate_value,
                v_total_order,
                'Transaccion de importacion generada por carga inicial',
                current_timestamp,
                null
            );
        end loop;
        ...
        call sp_log_proceso(
            'sp_cargar_operacion_etheria',
            'operacion',
            'OK',
            'Ordenes, detalles, trazabilidad, transacciones y estados de cuenta cargados',
            0
        );
    exception
        when others then
